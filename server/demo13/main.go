...
type Server13 struct {
	ConnMap  map[string]*Conn
	listener *net.TCPListener
}

func (s *Server13) Init(ip string, port int) error {
	var err error
...
func (s *Server13) wHandler(conn net.Conn, C *Conn) {
	// 讀取業務Work 寫入Wch的數據
...
// 讀客戶端數據 + 心跳檢測
func (s *Server13) rHandler(conn net.Conn, C *Conn) {
	// 心跳ack
	// 業務數據 寫入Wch
